             ...
             if (dsData.Tables.Count > 0)
             {
                 ...
                 if (dsData.Tables[1].Rows.Count > 0)
                 {
                    ...
                    dsData.Tables[1].Rows[0]["BackupDate"].ToString(), dsData.Tables[1].Rows[0]["Location"].ToString()
                          ,
                     "\n",
                                            dsData.Tables[1].Rows[0]["BackupType"].ToString()
                            )
         ...
         private void btnSave_Click(object sender, EventArgs e)
         {
             ...
             else
             {
               ...
               //  cmd.Parameters.AddWithValue("@DatabaseName", txtDbName.Text); // Your Database Name
                 cmd.Parameters.AddWithValue("@DatabaseName", ComboBoxDatabaseName.Text); // Your Database Name
         ...
         *//* WORK WITH TIMER *//*
         private void InitializeBackupTimer()
         {
             try
             {
                     ...
                     SqlCommand comandd = new SqlCommand(query, sqlCon);
                     object result = comandd.ExecuteScalar();
                     if (result != null && result != DBNull.Value)
                     {
                         int intervalDays = Convert.ToInt32(result);
                         backupTimer = new Timer();
                         //backupTimer.Interval = intervalDays * 24 * 60 * 60 * 1000; // Interval set to intervalDays days (converted to milliseconds)
                         backupTimer.Interval = 1000; // Interval set to intervalDays days (converted to milliseconds)
                         backupTimer.Tick += BackupTimer_Tick;
                         backupTimer.Start();
                         MessageBox.Show($" InitializeBackupTime successfully after  {intervalDays} day");
             ...
             try
             {
                 using (SqlConnection connection = new SqlConnection(conString))
                 {
                     ...
                     if (reader.Read())
                     {
                         ...
                         // Perform backup operations (create backup files)
                         for (int i = 1; i <= noOfFiles; i++)
                         {
                             string backupFileName = $"{location}\\{dbName}_Backup_{DateTime.Now.ToString("yyyyMMddHHmmss")}_{i}.bak";
                             // Perform backup using backupFileName
                             // Backup using SQL Server BACKUP DATABASE command
